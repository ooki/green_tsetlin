...
def make_pair_multi(x, y):
    ...
    p0 = np.array(i_set[:n//2])
    p1 = np.array(i_set[n//2:])
    
    xm = np.logical_or(x[p0], x[p1])
    
    e = np.eye(n_classes, dtype=int)
    ym = np.clip(e[y[p0]] + e[y[p1]], 0, 1)
    
    print("n_classes:", n_classes)
    
    return xm.astype(np.uint8), ym.astype(np.uint32)
...
def get_mnist():
    ...
    xt, yt = make_pair_multi(X_train, y_train)
    xe, ye = make_pair_multi(X_test, y_test)
    
    t1 = perf_counter()    
    delta = t1 - t0
    print("getting data time:{}".format(delta))
    print(xt.max(), xt.min(), yt.max(), yt.min())
...
if __name__ == "__main__":
    ...
    for n_jobs in [0]:
        ...
        for _ in range(n_repeats):        
            tm = gt.TsetlinMachine(n_literals=n_literals, n_clauses=n_clauses, n_classes=n_classes, s=s, n_literal_budget=n_literal_budget, multi_label=True)
